...
public class Oberflaeche
{
    ...
    public Oberflaeche() 
    {
        // Spieleranzahl per Dialog abfragen
        int spielerAnzahl = 0;
        while (spielerAnzahl < 2 || spielerAnzahl > 6) {
            String eingabe = JOptionPane.showInputDialog(
                null,
                "Wie viele Spieler? (2-6)",
                "Spieleranzahl wählen",
                JOptionPane.QUESTION_MESSAGE
            );
            if (eingabe == null) {
                System.exit(0); // Abbrechen gedrückt
            }
            try {
                spielerAnzahl = Integer.parseInt(eingabe.trim());
            } catch (NumberFormatException e) {
                spielerAnzahl = 0;
            }
        }
        brett = new Spielfeld(spielerAnzahl);
